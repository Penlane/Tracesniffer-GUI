...
class Head(Element):
    ...
    def drawText(self, event, qp):
        ...
        qp.drawText(event.rect(), QtCore.Qt.AlignLeft | QtCore.Qt.AlignBottom, str(self.main.min())+_unit)
...
class Tail(Element):
    ...
    def drawText(self, event, qp):
        qp.setPen(self.textColor())
        qp.setFont(QtGui.QFont('Arial', 10))
        qp.drawText(event.rect(), QtCore.Qt.AlignRight | QtCore.Qt.AlignTop, str(self.main.max())+_unit)
...
class Handle(Element):
    ...
    def drawText(self, event, qp):
        qp.setPen(self.textColor())
        qp.setFont(QtGui.QFont('Arial', 10))
        qp.drawText(event.rect(), QtCore.Qt.AlignLeft | QtCore.Qt.AlignBottom, str(self.main.start())+_unit)
        
        qp.drawText(event.rect(), QtCore.Qt.AlignRight | QtCore.Qt.AlignTop, str(self.main.end())+_unit)
    ...
    def mouseMoveEvent(self, event):
        ...
        if not _mx:
            ...
            dx = mx - _mx
        setattr(self, '__mx', mx)
        dt = (self.main.end() - self.main.start())/self.width()
        if dx == 0:
            event.ignore()
            return
        elif dx > 0:
            ...
            dx = dx*dt
        s = self.main.start() + int(dx)
        e = self.main.end() + int(dx)
