...
def gantt(chart, colors, title, bar_width, showgrid_x, showgrid_y, height,
    ...
    if group_tasks:
        task_names.reverse()
    try:
        task_names.remove("Event")
        task_names.insert(0,"Event")
    except ValueError:
        pass
    color_index = 0
    for index in range(len(tasks)):
        ...
        if tasks[index]['description']=="Ready":
            tasks[index].update(shape_template_for_ready_task)
        else:
        ...
        #Same tasks will get the same color 
        tasks[index]['fillcolor'] = colors[task_names.index(tn)%len(colors)]#colors[color_index]
